...
export class Preview {
  ...
  public ngOnInit(): void {
    ...
    this.form.forEach((control: FormcontrolInterface) => {
      ...
      if (control.type !== 'checkbox' && control.type !== 'layout')
        formGroup[control.name] = new FormControl(
          control.value ? control.value : '',
          controlValidators
        );
      else if (control.type === 'checkbox') {
        formGroup[control.name] = this.createCheckboxForm(
          control.checkboxOptions as CheckboxOptionsInterface[]
        );
        ...
        control.layout.forEach((item: LayoutFormcontrolInterface) => {
          ...
          if (item.validators) {
            item.validators.forEach((val: ValidatorInterface) => {
              if (val.validationName === 'required')
                layoutControlValidator.push(Validators.required);
              if (val.validationName === 'email')
                layoutControlValidator.push(Validators.email);
              if (val.validationName === 'maxlength')
                layoutControlValidator.push(
                  Validators.maxLength(val.maxLength as number)
                );
              if (val.validationName === 'minlength')
                layoutControlValidator.push(
          ...
          if (item.type !== 'checkbox')
            formGroup[item.name] = new FormControl(
              ...
              layoutControlValidator
            );
          else if (item.type === 'checkbox') {
            formGroup[item.name] = this.createCheckboxForm(
              item.checkboxOptions as CheckboxOptionsInterface[]
            );
          }
        });
      }
  ...
  getValidationError(
    control: FormcontrolInterface | LayoutFormcontrolInterface
  ): string {
